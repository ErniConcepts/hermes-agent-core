...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    # ── Version 10 → 11: narrow default CLI toolsets for product installs ──
    if current_ver < 11:
        config = load_config()
        changed = False
        ...
        if config.get("toolsets") == ["hermes-cli"]:
            ...
            results["config_added"].append(
                "toolsets=file,terminal,memory (replaces legacy hermes-cli default)"
            )
            changed = True
        ...
        if isinstance(platform_toolsets, dict) and platform_toolsets.get("cli") == ["hermes-cli"]:
            ...
            results["config_added"].append(
                "platform_toolsets.cli=file,terminal,memory (replaces legacy hermes-cli default)"
            )
            changed = True
