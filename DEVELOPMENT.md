...
- Product runtime tool access must be expressed directly as Hermes toolset names from `product.yaml`, not as a separate legacy tier/profile compatibility layer.
...
- End-user branding should come from setup-owned product config, not from hardcoded legacy branding.
